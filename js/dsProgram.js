...
function DegreesCommonClass() {
	...
	this.activeCssAnimation = function(index){
		var currentPage = $('.swiper-slide')[parseInt(index)];
		var els = $(currentPage).find("[data-animate-in]");
		$.each(els,function(i,o){
			var c_o = $(o);
			var init = c_o.data('animate-in');
			c_o.removeClass(init);
			c_o.addClass(init);
			var loop = c_o.data('animate-loop');
			if(loop){
				setTimeout(function(){
					c_o.removeClass(init);
					c_o.addClass(loop);
	...
	this.clearCssAnimation = function(index){
		var currentPage = $('.swiper-slide')[parseInt(index)];
		var els = $(currentPage).find("[data-animate-in]");
		$.each(els,function(i,o){
			var c_o = $(o);
	...
	this.submit_form = function(form,type){
		...
		switch (type){
			...
			break;
			case 2 : ctrl = 'wine';
				formData.append('active',$('#isShow').parent().hasClass('switch-on'));
                    ...
                    '<div class="row">',
						'<div class="col-md-12">',
							'<form action="" id="form_sample_1" class="form-horizontal dg_wine_form" method="post" enctype="multipart/form-data">',
								'<input name="wid" type="hidden" class="fd_pic_id" value="'+((id)?id:'')+'"/>',
								'<div class="tabbable-custom nav-justified">',
									'<div class="tab-pane active" id="">',
										'<div class="form-group">',
											'<label class="control-label col-md-3">酒名<span class="required">*</span></label>',
											'<div class="col-md-6">',
												'<input type="text" name="name_cn" data-required="1"  class=" form-control" value=""/>',
											'</div>',
										'</div>',
										'<div class="form-group">',
											'<label class="control-label col-md-3">外文名<span class="required">*</span></label>',
											'<div class="col-md-6">',
												'<input type="text" name="name_en" data-required="1"  class=" form-control" value=""/>',
											'</div>',
										'</div>',
			                            ...
			                            '<div class="form-group">',
											...
											'<div class="col-md-3">',
												'<input type="text" name="price" data-required="1"  class=" form-control" value=""/>',
											'</div>',
										'</div>',
										'<div class="form-group">',
											'<label class="control-label col-md-3">年份<span class="required">*</span></label>',
											'<div class="col-md-3">',
												'<input type="text" name="years" data-required="1"  class=" form-control" value=""/>',
											'</div>',
										'</div>',
											...
											'<div class="col-md-3">',
												'<div class="fileupload fileupload-new" data-provides="fileupload">',
													'<div class="fileupload-new thumbnail" style="width: 150px;">',
														'<img src="images/no_img.gif" alt="" class="small_prev_img"/>',
													'</div>',
										...
										'<div class="form-group">',
			                              ...
			                              '<div class="col-md-9">',
			                                 '<textarea class="ckeditor form-control" name="desc" id="desc" rows="6" data-error-container="#editor2_error"></textarea>',
										...
										'<div class="form-group">',
			                              ...
			                              '<div class="col-md-9">',
			                                 '<div class="make-switch has-switch" data-on="primary" data-off="info">',
			                                    '<input type="checkbox" name="active" checked class="toggle" id="isShow"/>',
			                                 '</div>',
			                              '</div>',
			                            '</div>',
